...
for ch = 1:C
    ...
    for fold_num = 1:CV_number
        ...
        for j=1:CV_number
            ...
            counter = counter+1;
            temp_label = true_label{1,fold_num}(this_set)' ;
            temp_label = temp_label(:)';

            %             lsim_hingeloss_traintest{ch, fold_num,counter} = hingeloss_traintest{ch,fold_num}(:,this_set) +1* randn(size( hingeloss_traintest{ch,fold_num}(:,this_set))) ;
            lsim_hingeloss_traintest{ch, fold_num,counter} = temp_label +(5+(0.5*ch))/5*randn(size(temp_label)) ;
            noise2 = abs((5+(0.5*ch))/5*randn(size(temp_label))) ;
            ind_rand = randperm(length(temp_label));
            lsim_hingeloss_traintest{ch, fold_num,counter}(ind_rand(1:floor(length(temp_label)/2))) = noise2(1:floor(length(temp_label)/2));
            if ch>8
...
[pi_0_lsim , coupling_tetha_convex_comb1 , transition_matrices_convex_comb1 ,  lsim_gmm_para ,  AIC , log_likelihood , BIC ,pi_steady,~, kl_dist, acc, ce_channels1] = ...
    lsim_supervised_fast( squeeze(lsim_hingeloss_traintest(:, fold_num, :)) , squeeze(channel_states(:, fold_num, :)) , num_gmm_component , max_itration , extra);
cmean1 = mean(coupling_tetha_convex_comb1,2);
extra.sup_learn_flag = 0;
[pi_0_lsim , coupling_tetha_convex_comb , transition_matrices_convex_comb ,  lsim_gmm_para ,  AIC , log_likelihood , BIC ,pi_steady,~, kl_dist, acc, ce_channels] = ...
    lsim_supervised_fast( squeeze(lsim_hingeloss_traintest(:, fold_num, :)) , squeeze(channel_states(:, fold_num, :)) , num_gmm_component , max_itration , extra);
cmean = mean(coupling_tetha_convex_comb,2);
...
for ch = 1:C
    for i = 1:CV_number
        ...
        for j=1:CV_number
            ...
            if ch>2
                lsim_hingeloss_traintest{ch, i,j} = randn(size(lsim_hingeloss_traintest{ch, i,j}));
            end
            %             temp_label = [temp_label(1),temp_label(1:end-1)];
