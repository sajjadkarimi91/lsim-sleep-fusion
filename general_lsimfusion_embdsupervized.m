...
for learn_flag=0:1
    ...
    for sigma_diag_loop =1
        ...
        for km = 1:length(model_name_all)
            ...
            for ch = 1:C
                ...
                for i = 1:CV_number
                    ...
                    for j=1:CV_number
                        ...
                        counter = counter+1;
                        temp_label = true_label{ch,i}(this_set)' ;
                        temp_label = temp_label(:)';

                        lsim_hingeloss_traintest{ch, i,counter} = hingeloss_traintest{ch,i}(:,this_set);
                        lsim_hingeloss_traintest{C+ch, i,counter} =[lsim_hingeloss_traintest{ch, i,counter}(:,2:end),lsim_hingeloss_traintest{ch, i,counter}(:,end)];
                        channel_states{ch, i,counter} = temp_label;
                        channel_states{C+ch, i,counter} = [temp_label(2:end),temp_label(end)];
                    end
            ...
            extra.sup_learn_flag =learn_flag;
            extra.auto_gmm = 1;
            num_gmm_component_grid = 5;

            counter = 0;

            ss = 1;
            num_gmm_component = ones(1,2*C)*num_gmm_component_grid(ss);
            parfor i = 1:CV_number
                ...
                [pi_0_lsim , coupling_tetha_convex_comb , transition_matrices_convex_comb ,  lsim_gmm_para ,  AIC , log_likelihood , BIC ,pi_steady] = ...
                    lsim_supervised_fast_embed( squeeze(lsim_hingeloss_traintest(:, i, :)) , squeeze(channel_states(:, i, :)) , num_gmm_component , max_itration , extra);
            ...
            save(['slsim_',num2str(channel_num),'ch_',model_name,num2str((learn_flag+1)*2+sigma_diag_loop),'.mat'],'lsim_gmm_para_all','transitions_matrices_all','coupling_tetha_all','pi_0_all','AIC_all','log_likelihood_all','BIC_all')
            ...
            for ch = 1:C
                for i = 1:CV_number
                    ...
                    for j=1:CV_number
                        ...
                        lsim_hingeloss_traintest{C+ch, i,j} =[lsim_hingeloss_traintest{ch, i,j}(:,2:end),lsim_hingeloss_traintest{ch, i,j}(:,end)];
            ...
            for repeat_num = 1:max_r
                ...
                for i = 1:CV_number
                    ...
                    for ss = 1:length(num_gmm_component_grid)
                        ...
                        for k=1:CV_number
                            ...
                            predictors = log(cell2mat(alpha_T_all(1:C,1))+10^-7);
                ...
                save(['slbss_',num2str(channel_num),'ch_',model_name,num2str((learn_flag+1)*2+sigma_diag_loop),'.mat'],'lbss','lbss_v')
            end
            ...
            save(['sres_',num2str(channel_num),'ch_',model_name,'_',num2str((learn_flag+1)*2+sigma_diag_loop),'.mat'],'kappa','acc','f1','acc2', 'kappa2' , 'f12','acc3', 'kappa3' , 'f13',"y_true","y_test","para_best","y_true2","y_test2","para_best2","y_true3","y_test3")
